...
def setupWebdriver(metamask_path, chrome_path=None, version=None, chromedriver_path=None):
    ...
    global wait_medium
    wait_medium = WebDriverWait(driver, 5, 1)
...
def createMeta(password):
    ...
    wait_fast.until(EC.invisibility_of_element_located(
    ...
    wait.until(EC.element_to_be_clickable(
        (By.CSS_SELECTOR, "button[data-testid='onboarding-create-wallet']"))).click()
    ...
    create_wallet = driver.find_element(
        By.CSS_SELECTOR, "button[data-testid='create-password-wallet']")
    create_wallet.click()

    # # find the submit button and click
    # submit_button = driver.find_element(
    #     By.CSS_SELECTOR, "button[data-testid='secure-wallet-later']")
    # submit_button.click()
    time.sleep(2)
    # find the submit button and click
    secure_now_btn = driver.find_element(
        By.CSS_SELECTOR, "button[data-testid='secure-wallet-recommended']")
    secure_now_btn.click()

    # find the submit button and click
    reveal_now_btn = driver.find_element(
        By.CSS_SELECTOR, "button[data-testid='recovery-phrase-reveal']")
    reveal_now_btn.click()

    # find the submit button and click
    copy_clip = driver.find_element(
        By.XPATH, "(//span[@class='button__icon'])[2]")
    copy_clip.click()
    ...
    next_btn = driver.find_element(
        By.CSS_SELECTOR, "button[data-testid='recovery-phrase-next']")
    next_btn.click()
    # wait.until(EC.element_to_be_clickable(
    #     (By.CSS_SELECTOR, "input[data-testid='skip-srp-backup-popover-checkbox']"))).click()
    #
    # skip_btn = driver.find_element(
    #     By.CSS_SELECTOR, "button[data-testid='skip-srp-backup']")
    # skip_btn.click()
...
def switch_network():
    ...
    try:
        # This button is only available when the popup is closed
        wait.until(EC.element_to_be_clickable(
            (By.CSS_SELECTOR, "button[data-testid='confirmation-submit-button']"))).click()
    except Exception:
        logging.error("Connect wallet failed")
        return
...
def approve():
    ...
    try:
        wait_fast.until(EC.element_to_be_clickable(
            (By.CSS_SELECTOR, "input[id='custom-spending-cap']"))).send_keys(str(1000))
    except Exception:
        print('no max')
        print(Exception)
        driver.refresh()
...
def confirm():
    ...
    try:
        wait_fast.until(EC.element_to_be_clickable(
            (By.CSS_SELECTOR, "div[data-testid='signature-request-scroll-button']"))).click()
    except Exception:
        print('No scroll')
        print(Exception)
        driver.refresh()
